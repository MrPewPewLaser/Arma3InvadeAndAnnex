    ...
    0 = [] spawn {
...

        {
            if (!isNil {missionNamespace getVariable _x}) then {
                player disableUAVConnectability [(missionNamespace getVariable _x), true];
            };
        } forEach ["Base_AA", "CVN_CIWS_1", "CVN_CIWS_2", "CVN_CIWS_3", "CVN_RAM", "CVN_SAM_2", "CVN_SAM_3"];
    };
};
...
inGameUISetEventHandler ["Action", (toString {
    ...
    if (_caller getVariable ["derp_revive_downed", false]) exitWith {
    ...
    switch (_engineName) do {
        case "UAVTerminalHackConnection": {
            if !(_target getVariable ["InA_UAV_CanBeHacked", false]) then {
                ...
                systemChat "This drone cannot be hacked ....";
            };
        };
    };
