...
if (_HqBuilding isEqualTo objNull) exitWith {
    {_x call AW_fnc_delete} forEach _buildings;
};
...
_HqBuilding animateSource ["Hatch_1_source", 0];

private _side = (_factionHash get "#side");

private _garrisongroup = createGroup _side;
_garrisongroup setGroupIdGlobal ["AO-SubObj-HQ"];
private _officerType = ["#officers", _factionHash] call AW_fnc_getUnitsFromHash;

private _officer = _garrisongroup createUnit [_officerType, _officerPos, [], 0, "CAN_COLLIDE"];
...
_officer setVariable ["HQ_Building", _HqBuilding];

_officer addEventHandler ["Killed",{
    params ["_unit","","_killer"];
    private _hq = _unit getVariable ["HQ_Building", objNull];
...
_HqBuilding addEventHandler ["Killed",{
    ...
    [_hqPos] spawn {
        ...
        sleep 1;
        private _hqRuins = (nearestObject [_pos, "Land_Cargo_HQ_V1_ruins_F"]);

        InA_Server_subObjUnits pushBack _hqRuins;
    };
}];
...
[
    ...
    [
        "OPFOR have set up an HQ building in the AO. Inside is an officer, neutralise him using any force necessary.  After that destroy the building.",
        "HQ Building",
        "radioMarker"
    ],
...
[(units _garrisongroup)] call AW_fnc_addToAllCurators;
...
InA_Server_subObjUnits append ((units _garrisongroup) + _objArray);

waitUntil {
    if (InA_Server_MainAOControl_EndSubObj) exitWith {
        {
            if (alive _x) then {_x setDamage 1};
        } forEach [_officer, _HqBuilding];
    ...
    ((!alive _officer) && (!alive _HqBuilding))
};
